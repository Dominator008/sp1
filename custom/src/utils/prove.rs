...
#[derive(Error, Debug)]
pub enum CustomCoreProverError {
...
) -> Result<MachineProof<SC>, CustomCoreProverError>
where
    SC::Challenger: Clone,
...
{
    // Setup the machine.
    let machine = CustomAir::machine(config);
...
pub fn prove<SC: StarkGenericConfig + Send + Sync>(
    ...
    config: SC,
    opts: SP1CoreOpts,
) -> Result<(MachineProof<SC>, Vec<u8>), CustomCoreProverError>
where
    SC::Challenger: Clone,
    OpeningProof<SC>: Send + Sync,
    Com<SC>: Send + Sync,
    ...
    <SC as StarkGenericConfig>::Val: PrimeField32,
{
    ...
    // Create the runtime
    let mut runtime = CustomRuntime::new(program.clone());
    ...
    runtime
        .run()
        .map_err(CustomCoreProverError::ExecutionError)?;
...
pub fn run_test_io(
    program: Program,
    inputs: SP1Stdin,
) -> Result<SP1PublicValues, sp1_core::stark::MachineVerificationError<BabyBearPoseidon2>> {
...
pub fn run_test(
    ...
    sp1_core::stark::MachineProof<BabyBearPoseidon2>,
    sp1_core::stark::MachineVerificationError<BabyBearPoseidon2>,
> {
    let runtime = tracing::info_span!("runtime.run(...)").in_scope(|| {
        let mut runtime = Runtime::new(program, SP1CoreOpts::default());
        runtime.run().unwrap();
...
) -> Result<sp1_core::stark::MachineProof<SC>, sp1_core::stark::MachineVerificationError<SC>>
where
    A: MachineAir<SC::Val>
