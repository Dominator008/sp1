...
use sp1_core::{
    runtime::Program,
    stark::{LocalProver, ShardProof, StarkGenericConfig, StarkMachine, StarkVerifyingKey, Val},
    utils::{BabyBearPoseidon2, SP1CoreProverError},
};
use sp1_custom::utils::CustomCoreProverError;
use sp1_custom::{runtime::CustomProgram, stark::CustomAir};
use sp1_custom_recursion_circuit::witness::Witnessable;
use sp1_custom_recursion_compiler::config::InnerConfig;
use sp1_custom_recursion_compiler::ir::Witness;
use sp1_custom_recursion_core::{
    air::RecursionPublicValues,
    runtime::{RecursionProgram, Runtime as RecursionRuntime},
    stark::{config::BabyBearPoseidon2Outer, RecursionAir},
};
pub use sp1_custom_recursion_gnark_ffi::plonk_bn254::PlonkBn254Proof;
use sp1_custom_recursion_gnark_ffi::plonk_bn254::PlonkBn254Prover;
use sp1_custom_recursion_program::hints::Hintable;
pub use sp1_custom_recursion_program::machine::ReduceProgramType;
use sp1_custom_recursion_program::machine::{
    SP1CompressVerifier, SP1DeferredVerifier, SP1RecursiveVerifier, SP1RootVerifier,
};
pub use sp1_custom_recursion_program::machine::{
...
/// A end-to-end prover implementation for the custom circuit
pub struct CustomProver {
    ...
    pub core_machine: StarkMachine<CoreSC, CustomAir<<CoreSC as StarkGenericConfig>::Val>>,
...
impl CustomProver {
    ...
    pub fn new() -> Self {
        let core_machine = CustomAir::machine(CoreSC::default());
    ...
    pub fn setup(&self, elf: &[u8]) -> (CustomProvingKey, CustomVerifyingKey) {
        let program = CustomProgram {};
        let (pk, vk) = self.core_machine.setup(&program);
        let vk = CustomVerifyingKey { vk };
        let pk = CustomProvingKey {
            pk,
            elf: elf.to_vec(),
            vk: vk.clone(),
        };
    ...
    pub fn prove_core(
        ...
        pk: &CustomProvingKey,
        stdin: &SP1Stdin,
    ) -> Result<CustomCoreProof, CustomCoreProverError> {
        let config = CoreSC::default();
        let program = CustomProgram {};
        let (proof, public_values_stream) =
            sp1_custom::utils::prove(program, stdin, config, self.core_opts)?;
        let public_values = SP1PublicValues::from(&public_values_stream);
        Ok(CustomCoreProof {
            proof: CustomCoreProofData(proof.shard_proofs),
            stdin: stdin.clone(),
            public_values,
        })
    }
    ...
    pub fn get_recursion_core_inputs<'a>(
        ...
        batch_size: usize,
        is_complete: bool,
    ) -> Vec<SP1RecursionMemoryLayout<'a, CoreSC, CustomAir<BabyBear>>> {
    ...
    pub fn get_first_layer_inputs<'a>(
        &'a self,
        vk: &'a CustomVerifyingKey,
        leaf_challenger: &'a Challenger<InnerSC>,
        shard_proofs: &[ShardProof<InnerSC>],
        deferred_proofs: &[ShardProof<InnerSC>],
        batch_size: usize,
    ) -> (
        Vec<SP1RecursionMemoryLayout<'a, InnerSC, CustomAir<BabyBear>>>,
        Vec<SP1DeferredMemoryLayout<'a, InnerSC, RecursionAir<BabyBear, 3>>>,
    ) {
    ...
    pub fn compress(
        &self,
        vk: &CustomVerifyingKey,
        proof: CustomCoreProof,
        deferred_proofs: Vec<ShardProof<InnerSC>>,
    ) -> Result<CustomReduceProof<InnerSC>, CustomRecursionProverError> {
        ...
        for inputs in core_inputs.chunks(shard_batch_size) {
            let proofs = inputs
                ...
                .map(|input| {
                    ...
                    tracing::info!("calling self.compress_machine.prove");
                    (
    ...
    pub fn shrink(
        &self,
        reduced_proof: CustomReduceProof<InnerSC>,
    ) -> Result<CustomReduceProof<InnerSC>, CustomRecursionProverError> {
    ...
    pub fn wrap_bn254(
        &self,
        compressed_proof: CustomReduceProof<InnerSC>,
    ) -> Result<CustomReduceProof<OuterSC>, CustomRecursionProverError> {
        let input = SP1RootMemoryLayout {
    ...
    pub fn wrap_plonk_bn254(
        &self,
        proof: CustomReduceProof<OuterSC>,
        build_dir: &Path,
    ) -> PlonkBn254Proof {
...
mod tests {
    ...
    fn test_e2e() -> Result<()> {
        ...
        let mut prover = CustomProver::new();
