...
use sp1_custom_recursion_gnark_ffi::plonk_bn254::PlonkBn254Proof;
...
impl HashableKey for CustomVerifyingKey {
    fn hash_babybear(&self) -> [BabyBear; DIGEST_SIZE] {
...
pub type CustomCoreProof = SP1ProofWithMetadata<CustomCoreProofData>;

/// An SP1 proof that has been recursively reduced into a single proof. This proof can be verified
/// within SP1 programs.
pub type CustomReducedProof = SP1ProofWithMetadata<CustomReducedProofData>;

/// An SP1 proof that has been wrapped into a single PLONK proof and can be verified onchain.
pub type CustomPlonkBn254Proof = SP1ProofWithMetadata<CustomPlonkBn254ProofData>;

/// An SP1 proof that has been wrapped into a single PLONK proof and can be verified onchain.
pub type SP1PlonkProof = SP1ProofWithMetadata<CustomPlonkProofData>;

#[derive(Serialize, Deserialize, Clone)]
pub struct CustomCoreProofData(pub Vec<ShardProof<CoreSC>>);
#[derive(Serialize, Deserialize, Clone)]
pub struct CustomReducedProofData(pub ShardProof<InnerSC>);

#[derive(Serialize, Deserialize, Clone)]
pub struct CustomPlonkBn254ProofData(pub PlonkBn254Proof);

#[derive(Serialize, Deserialize, Clone)]
pub struct CustomPlonkProofData(pub PlonkBn254Proof);
...
pub struct CustomReduceProof<SC: StarkGenericConfig> {
    pub proof: ShardProof<SC>,
}

impl CustomReduceProof<BabyBearPoseidon2Outer> {
...
#[derive(Serialize, Deserialize, Clone)]
pub enum CustomReduceProofWrapper {
    Core(CustomReduceProof<CoreSC>),
    Recursive(CustomReduceProof<InnerSC>),
}

#[derive(Error, Debug)]
pub enum CustomRecursionProverError {}
