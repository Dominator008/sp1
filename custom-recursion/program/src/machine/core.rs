...
use sp1_custom_recursion_compiler::ir::{Array, Builder, Config, Ext, ExtConst, Felt, Var};
use sp1_custom_recursion_compiler::prelude::DslVariable;
use sp1_custom_recursion_core::air::{RecursionPublicValues, RECURSIVE_PROOF_NUM_PV_ELTS};
use sp1_custom_recursion_core::runtime::{RecursionProgram, DIGEST_SIZE};
...
impl SP1RecursiveVerifier<InnerConfig, BabyBearPoseidon2> {
    ...
    pub fn build(
        machine: &StarkMachine<BabyBearPoseidon2, CustomAir<BabyBear>>,
    ) -> RecursionProgram<BabyBear> {
        let mut builder = Builder::<InnerConfig>::default();

        let input: SP1RecursionMemoryLayoutVariable<_> = builder.uninit();
        SP1RecursionMemoryLayout::<BabyBearPoseidon2, CustomAir<_>>::witness(&input, &mut builder);
    ...
    Com<SC>: Into<[SC::Val; DIGEST_SIZE]>,
{
    ...
    pub fn verify(
        ...
        machine: &StarkMachine<SC, CustomAir<SC::Val>>,
